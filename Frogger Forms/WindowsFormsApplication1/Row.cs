    ...
    class Row : frogger.Object
    {
        ...
        private void addObject()
        {
            if ((speed > 0 && objects[objects.Count - 1].getPosition().X > 0)
                ||
                 (speed < 0 && objects[objects.Count - 1].getPosition().X + objects[objects.Count - 1].getWidth() > Form1.width))
            {
                objects.Add(new Object(speed < 0 ? Form1.width : 0, (int)position.Y));
                setObjectSprite();
        ...
        public override void update(float time = .01666f)
        {
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                ...
                else if (speed > 0 && objects[i].getPosition().X > Form1.width)
                {
                    objects.RemoveAt(i);
                    objects.TrimExcess();
                ...
                else if (speed < 0 && objects[i].getPosition().X + objects[i].getWidth() > Form1.width)
                {
            ...
            if (createObject && Form1.rand.NextDouble() < .005 * Math.Abs(speed))
            {
                addObject();
            }
        }
